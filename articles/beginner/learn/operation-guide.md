...
※　オブジェクトは画面の中心に配置されます。  
　　地面より下には配置されないため、**カメラを下の方に向けてオブジェクトを配置すると、y軸0の接地状態で配置することが出来ます。**  
...
ノードグラフ編集画面では、右クリックでノードライブラリを開き、そこから新規ノードを作成出来ます。  
検索ボックスからノードを検索する際、**文字列確定後にEnterを押してしまうと、ノード一覧が閉じてしまう**ので注意してください。  
